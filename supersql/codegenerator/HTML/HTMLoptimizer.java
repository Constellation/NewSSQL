...
public class HTMLoptimizer {
	...
	public String generateHtml( String filename) {	
		try{
			...
			if (del_file.exists()) {
				...
				System.err.println(del_file + "does not exist");
			}
			return startOptimizer();
		}catch(Exception e){
			System.err.println("XML File not found : " + e);
		...
		ch != null;ch = ch.getNextSibling()){
			if(ch.getNodeType() == Node.ELEMENT_NODE){
				...
				if(nowNode.getAttribute("type").equals("connect") 
						...
						&& parentNode.getAttribute("dimension").equals(nowNode.getAttribute("dimension"))){

					if(!nowNode.getAttribute("tabletype").equals("nest")){
						Node brother = ch.getNextSibling();
						for(Node ch2 = ch.getFirstChild();
				...
				if( nowNode.getAttribute("type").equals("repeat")
						&& parentNode.getAttribute("type").equals("connect")
						&& nowNode.getAttribute("dimension").equals(parentNode.getAttribute("dimension"))){
					if( !nowNode.getAttribute("tabletype").equals("nest")){
						if(nowNode.hasAttribute("border")){
							for(Node tmp=ch.getFirstChild();tmp !=null;tmp=tmp.getNextSibling()){
								Element childNode = (Element)tmp;
								childNode.setAttribute("border",nowNode.getAttribute("border"));
							}
						}
						Node brother = ch.getNextSibling();
						for(Node ch2 = ch.getFirstChild();
				...
				if( nowNode.getAttribute("type").equals("repeat")){
					...
					ch2 != null;ch2 = ch.getFirstChild()){
						...
						if(ch.getPreviousSibling() != null && ch.getPreviousSibling().getNodeName().equals("tfe")){
							previousNode = (Element)ch.getPreviousSibling();
						}

						if((previousNode != null && previousNode.getAttribute("dimension").equals(nowNode.getAttribute("dimension"))
								&& previousNode.getAttribute("type").equals("connect")&& previousNode.getAttribute("oldtype").equals("repeat"))){
							if(nowNode.hasAttribute("border")){
								for(Node tmp=ch.getFirstChild();tmp !=null;tmp=tmp.getNextSibling()){
									Element childNode = (Element)tmp;
									childNode.setAttribute("border",nowNode.getAttribute("border"));
							...
							if(nowNode.hasAttribute("class")){
								strtfe.setAttribute("class",nowNode.getAttribute("class"));
							}
							if(nowNode.hasAttribute("cellspacing")){
								strtfe.setAttribute("cellspacing",nowNode.getAttribute("cellspacing"));
							}
							if(nowNode.hasAttribute("cellpadding")){
								strtfe.setAttribute("cellpadding",nowNode.getAttribute("cellpadding"));
							}
							if(nowNode.hasAttribute("align")){
								strtfe.setAttribute("align",nowNode.getAttribute("align"));
							}
							if(nowNode.hasAttribute("valign")){
								strtfe.setAttribute("valign",nowNode.getAttribute("valign"));
							}
							if(nowNode.hasAttribute("border")){
								strtfe.setAttribute("border",nowNode.getAttribute("border"));
								for(Node tmp=ch.getFirstChild();tmp !=null;tmp=tmp.getNextSibling()){
									Element childNode = (Element)tmp;
									childNode.setAttribute("border",nowNode.getAttribute("border"));
								}
							...
							nowNode = (Element) ch ;
							strtfe.setAttribute("dimension",nowNode.getAttribute("dimension"));
	...
	public void walk(Node n){
		...
		ch != null;ch = ch.getNextSibling()){
			if(ch.getNodeType() == Node.ELEMENT_NODE){
				Element nowNode = (Element) ch ;
				tmp++;
				if(ch.getNodeName().equals("VALUE") || ch.getNodeName().equals("EMBED")){
					nowNode.setAttribute("frontval",Integer.toString(inval+frval));
					inval++;
				}else{
					nowNode.setAttribute("child",Integer.toString((ch.getChildNodes()).getLength()));
					nowNode.setAttribute("frontval",Integer.toString(inval+frval));
					walk(ch);
				}
				if((nowNode.getAttribute("type").equals("connect"))){
		...
		ch != null;ch = ch.getNextSibling()){
			if(ch.getNodeType() == Node.ELEMENT_NODE){
				...
				if((nowNode.getAttribute("type").equals("connect"))
						...
						&& (Integer.parseInt(nowNode.getAttribute("child")) == value)){//C1で子に値しかないとき
					if(parentNode.getAttribute("type").equals("connect") &&
							parentNode.getAttribute("dimension").equals("2")){//親がC2
						int max;
						if(!parentNode.hasAttribute("td")){//nullなら
							max = value;
						}else{
							int tmp = Integer.parseInt(parentNode.getAttribute("td"));
							max = lcm(tmp,value);
						}
						if(nowNode.getAttribute("tabletype").equals("nest")){
							...
							nowNode.setAttribute("oldtd",Integer.toString(value));
						}
					}
				}else if(nowNode.getAttribute("type").equals("connect") &&
						nowNode.getAttribute("dimension").equals("1")){//子に値以外のものがあるとき
					walkC1(ch);
					int max;
					if(!nowNode.hasAttribute("td")){//nullなら
						max = value;
						for(Node ch2 = ch.getFirstChild();
						ch2 != null;ch2 = ch2.getNextSibling()){
							...
							if(childNode.getAttribute("type").equals("connect")){
								if(childNode.getAttribute("tabletype").equals("nest")){
									...
									max += Integer.parseInt(childNode.getAttribute("td"));
									childNode.setAttribute("oldtd",childNode.getAttribute("td"));
									childNode.removeAttribute("td");
								}
							}
						}
					}else{
						max = value + Integer.parseInt(nowNode.getAttribute("td"));
					}
					
					nowNode.setAttribute("td",Integer.toString(max));
					if(parentNode.getAttribute("type").equals("connect") &&
							parentNode.getAttribute("dimension").equals("1")){ // 親がC1のとき
						if(nowNode.getAttribute("tabletype").equals("nest")){
							max++;
						}else if(parentNode.hasAttribute("td")){
							max = max + Integer.parseInt(parentNode.getAttribute("td"));
						}
						parentNode.setAttribute("td",Integer.toString(max));
					}
				}else{// if node is not C1 -> next children 
					walkC1(ch);					
					for(Node ch2 = ch.getFirstChild();//子供をたどりそのなかのtdをもちあげる
					ch2 != null;ch2 = ch2.getNextSibling()){
						Element childNode;
						if(ch2.getNodeType() == Node.ELEMENT_NODE){
							childNode = (Element)ch2;
						...
						if(childNode.getAttribute("type").equals("connect")){
							if(childNode.hasAttribute("td")){//こどもにtdがあるなら
								int tmp=0;
								if(!nowNode.hasAttribute("td")){//nowNodeのtdがヌルなら
									tmp = Integer.parseInt(childNode.getAttribute("td"));

								}else{//ヌルじゃないなら最小公倍数
									if(nowNode.getAttribute("type").equals("connect")
											&& nowNode.getAttribute("dimension").equals("2")){//C2ならば
										if(!childNode.getAttribute("tabletype").equals("nest")){
											tmp = lcm(Integer.parseInt(nowNode.getAttribute("td")),Integer.parseInt(childNode.getAttribute("td")));								
										}else{
											tmp = Integer.parseInt(nowNode.getAttribute("td"));
										}
									}
								}
								if(nowNode.hasAttribute("td")){
									int aaa = Integer.parseInt(nowNode.getAttribute("td"));
									int tmp2 = lcm(tmp,aaa);
									nowNode.setAttribute("td",Integer.toString(tmp2));
								}else{
									nowNode.setAttribute("td",Integer.toString(tmp));
								}
								if(!childNode.getAttribute("tabletype").equals("nest")){
									childNode.setAttribute("oldtd",childNode.getAttribute("td"));
									childNode.removeAttribute("td");
						...
						if(ch2.getNodeName().equals("VALUE") || ch2.getNodeName().equals("EMBED")){
							if(nowNode.hasAttribute("td")){//nowNodeにtdがある
								int tmp = 0;
								if(nowNode.getAttribute("type").equals("connect") &&
										nowNode.getAttribute("dimension").equals("2") ){//C2なら
									tmp = Integer.parseInt(nowNode.getAttribute("td"));
								}
								if(nowNode.getAttribute("type").equals("connect") &&
										nowNode.getAttribute("dimension").equals("1") ){//C1なら
									tmp = Integer.parseInt(nowNode.getAttribute("td"))+1;
								}
								nowNode.setAttribute("td",Integer.toString(tmp));
							}else{
				...
				if(nowNode.getAttribute("tabletype").equals("nest") && nowNode.hasAttribute("oldtd")){
		...
		if(n.getNodeName().equals("SSQL")){
			...
			for(Node ch = n.getFirstChild();ch != null;ch=ch.getNextSibling()){
				if(ch.getNodeType() == Node.ELEMENT_NODE){
					...
					if(childNode.hasAttribute("td")){
						int tmp = Integer.parseInt(childNode.getAttribute("td"));
						if(tmp>max)
							max = tmp;
					}
				}
			...
			for(Node ch = n.getFirstChild();ch != null;ch=ch.getNextSibling()){
				if(ch.getNodeType() == Node.ELEMENT_NODE){
					Element childNode = (Element) ch;
					if(childNode.hasAttribute("td")){
						if(!childNode.getAttribute("td").equals(Integer.toString(max))){
							int tmp = Integer.parseInt(childNode.getAttribute("td"));
							tmp = max - tmp;
							childNode.setAttribute("plustd",Integer.toString(tmp));
						}
	...
	public void countTrAndTd(Node n){
		for(Node ch = n.getFirstChild();
			...
			if(ch.getNodeName().equals("VALUE") || ch.getNodeName().equals("EMBED")){//値のとき
				//値のときはtr=1 td=1である
				Element nowNode = (Element) ch;
				nowNode.setAttribute("tr",Integer.toString(1));
				nowNode.setAttribute("td",Integer.toString(1));	
			}else if(ch.getNodeType() == Node.ELEMENT_NODE){//値ではないノードのとき
				//再帰的に呼び出して下から上
				countTrAndTd(ch);
				...
				if((nowNode.getAttribute("type").equals("connect"))
					...
					for(Node child = ch.getFirstChild();
					child != null;child = child.getNextSibling()){
						Element childNode = (Element) child;
						if(childNode.hasAttribute("td")){//attr:tdがあるとき
							//nestのときの処理
							if(childNode.getAttribute("tabletype").equals("nest")){
								valueTd++;
								//plus 1
							}else{
								valueTd += Integer.parseInt(childNode.getAttribute("td"));
								//子ノードのTdの足し合わせ
							}
						}
						if(childNode.hasAttribute("tr")){
							...
							if(childNode.getAttribute("tabletype").equals("nest")){
								...
								tmp = Integer.parseInt(childNode.getAttribute("tr"));
								//子ノードのTRを持ってくる
							}
							valueTr = lcm(tmp,valueTr);//さいしょうこうばいすう！
						}
						if(!childNode.getAttribute("tabletype").equals("nest") && !ch.getNodeName().equals("SSQL")){
							childNode.setAttribute("oldtd",childNode.getAttribute("td"));
							childNode.setAttribute("oldtr",childNode.getAttribute("tr"));
							childNode.removeAttribute("td");
				...
				if((nowNode.getAttribute("type").equals("connect"))
					...
					child != null;child = child.getNextSibling()){
						Element childNode = (Element) child;
						if(childNode.hasAttribute("tr")){//attr:tdがあるとき
							//nestのときの処理
							if(childNode.getAttribute("tabletype").equals("nest")){
								valueTr++;
								//plus 1
							}else{
								valueTr += Integer.parseInt(childNode.getAttribute("tr"));
								//子ノードのTrの足し合わせ
							}
						}
						if(childNode.hasAttribute("td")){
							...
							valueTd = lcm(tmp,valueTd);//さいしょうこうばいすう！
						}
						if(!childNode.getAttribute("tabletype").equals("nest") && !ch.getNodeName().equals("SSQL")){
							childNode.setAttribute("oldtd",childNode.getAttribute("td"));
							childNode.setAttribute("oldtr",childNode.getAttribute("tr"));
	...
	public void createTable(Node n,Node h){
		...
		ch != null;ch = ch.getNextSibling()){
			if(ch.getNodeType() == Node.ELEMENT_NODE){
				...
				//親、その前の兄弟をおっていく
				for(Node ch2=ch.getParentNode();ch2 != null;ch2 = ch2.getParentNode()){
					Element parentNode;
					if(ch2.getNodeType() == Node.ELEMENT_NODE){
					...
					int fr = 0;
					//もし前に兄弟がいたらそのぶんだけ挿入位置をずらす。
					if(parentNode.hasAttribute("frontval")){
						for(Node tmp=parentNode.getPreviousSibling();tmp != null;tmp=tmp.getPreviousSibling()){
							...
							if(brotherNode.hasAttribute("colspan") || brotherNode.hasAttribute("rowspan")){
								if(tmp2.getAttribute("dimension").equals("1") && !brotherNode.getAttribute("colspan").equals("")){
									fr += Integer.parseInt(brotherNode.getAttribute("colspan"));
								}else if(tmp2.getAttribute("dimension").equals("2") && !brotherNode.getAttribute("rowspan").equals("")){
									fr += Integer.parseInt(brotherNode.getAttribute("rowspan"));
								}
							}else{
								fr++;
							}
					...
					if(grandP != null && grandP.getAttribute("type").equals("connect") &&
							grandP.getAttribute("dimension").equals("2") ){//そのおやがC2のとき
						y += fr;
					}
				}
				//いまのノードの前のノード
				for(Node ch2=ch.getPreviousSibling();
				ch2 != null 
				&& ch2.getNodeName() != "#document"
					&& ch2.getNodeName() != "#text";
				ch2 = ch2.getPreviousSibling()){
					int fr = 0;
					Element brotherNode = (Element) ch2;
					if(ch2.getNodeName().equals("VALUE") && brotherNode.getAttribute("tabletype").equals("nest")){
						...
						if(parentNode.getAttribute("type").equals("connect") &&
							...
							if(brotherNode.hasAttribute("rowspan")){
								y += Integer.parseInt(brotherNode.getAttribute("rowspan"));
							}
						}
					}else if(brotherNode.getAttribute("type").equals("connect") &&
							brotherNode.getAttribute("dimension").equals("1") ){//きょうだいC1のとき
						if(brotherNode.hasAttribute("rowspan") && brotherNode.getAttribute("rowspan")!="" && !brotherNode.getAttribute("tabletype").equals("nest")){
							fr = Integer.parseInt(brotherNode.getAttribute("rowspan"));
						}
						y += fr;
					}else if(brotherNode.getAttribute("type").equals("connect") &&
							brotherNode.getAttribute("dimension").equals("2")){//きょうだいC2のとき
						if(brotherNode.hasAttribute("colspan") && brotherNode.getAttribute("colspan")!="" && !brotherNode.getAttribute("tabletype").equals("nest")){
							fr = Integer.parseInt(brotherNode.getAttribute("colspan"));
						}
					}
				}
				...
				Element parentNode = (Element) ch.getParentNode();
				if(parentNode.getAttribute("type").equals("connect") &&
					parentNode.getAttribute("dimension").equals("2")){/*C2 VALUE*/
					y+=valuenum;
				}

				if(nowNode.getAttribute("tabletype").equals("nest") && !ch.getNodeName().equals("VALUE")){
					a[num] = x;
					b[num] = y;
					num++;
				}	
				...
				*/
				...
				if(nowNode.getAttribute("type").equals("connect") &&
						nowNode.getAttribute("dimension").equals("1")){//nodeがC1のとき
					//childnodeのrowspan:現在のノードのrowspan(無い時はtr)
					//childnodeのcolspan:
					//・現在のノードがcolspanをもたないときは子ノードtd
					//・子ノードのtdの総和が現在のノードのcolspanのときは子ノードtd
					//・それ以外のときは式
					for(Node tmp=ch.getFirstChild();tmp != null;tmp = tmp.getNextSibling()){
						Element childNode = (Element) tmp;
						int maxtd = 0;
						if(nowNode.hasAttribute("tr")){
							childNode.setAttribute("rowspan",nowNode.getAttribute("tr"));
							maxtd = Integer.parseInt(nowNode.getAttribute("td"));
						}else if(nowNode.hasAttribute("rowspan")){
							childNode.setAttribute("rowspan",nowNode.getAttribute("rowspan"));
							maxtd = Integer.parseInt(nowNode.getAttribute("colspan"));
						}
						int needTd = 0;
						int nowTd = 0;
						for(Node tmp2 = ch.getFirstChild();tmp2 != null;tmp2 = tmp2.getNextSibling()){
							Element brotherNode = (Element)tmp2;
							if(tmp2 == tmp){
								if(brotherNode.hasAttribute("oldtd") && !brotherNode.getAttribute("oldtd").isEmpty()){
									nowTd += Integer.parseInt(brotherNode.getAttribute("oldtd"));
								}else if(brotherNode.getAttribute("tabletype").equals("nest")){
									nowTd++;
								}else if(tmp2.getNodeName().equals("VALUE")||tmp2.getNodeName().equals("EMBED")){
									nowTd++;
								}
								continue;
							}
							if(brotherNode.hasAttribute("oldtd") && !brotherNode.getAttribute("oldtd").isEmpty()){
								needTd += Integer.parseInt(brotherNode.getAttribute("oldtd"));
							}else if(brotherNode.getAttribute("tabletype").equals("nest")){
								needTd ++;
							}else if(tmp2.getNodeName().equals("VALUE")||tmp2.getNodeName().equals("EMBED")){
								needTd++;
						...
						if(nowTd + needTd == maxtd){
							if(childNode.hasAttribute("oldtd"))
								childNode.setAttribute("colspan",childNode.getAttribute("oldtd"));
							else
							...
							if(nowNode.hasAttribute("oldtd")){
								oldTd = Integer.parseInt(nowNode.getAttribute("oldtd"));
							}
							if(childNode.hasAttribute("oldtd")){
								choldTd = Integer.parseInt(childNode.getAttribute("oldtd"));
							}
							int set = maxtd/oldTd * choldTd;
							childNode.setAttribute("colspan",Integer.toString(set));
							//Log.out(maxtd +" "+ oldTd + " " +choldTd +"set colspan is " +set);
						}	
					}
				}
				if(nowNode.getAttribute("type").equals("connect") &&
						nowNode.getAttribute("dimension").equals("2")){//nodeがC2のとき
					//childnodeのrowspan:
					//・現在のノードがrowspanをもたないときは子ノードtr
					//・子ノードのtrの総和が現在のノードのrowspanのときは子ノードtr
					//・それ以外のときは式
					//childnodeのcolspan:現在のノードのcolspan(無い時はtd)
					for(Node tmp=ch.getFirstChild();tmp != null;tmp = tmp.getNextSibling()){
						Element childNode = (Element) tmp;;
						int maxtr = 0;
						if(nowNode.hasAttribute("td")){
							childNode.setAttribute("colspan",nowNode.getAttribute("td"));
							maxtr = Integer.parseInt(nowNode.getAttribute("tr"));
						}else if(nowNode.hasAttribute("colspan")){
							childNode.setAttribute("colspan",nowNode.getAttribute("colspan"));
							maxtr = Integer.parseInt(nowNode.getAttribute("rowspan"));
						}
						...
						for(Node tmp2 = ch.getFirstChild();tmp2 != null;tmp2 = tmp2.getNextSibling()){
							Element brotherNode = (Element)tmp2;
							if(tmp2 == tmp){
								if(brotherNode.hasAttribute("oldtr") && !brotherNode.getAttribute("oldtr").equals("")){
									nowTr += Integer.parseInt(brotherNode.getAttribute("oldtr"));
								}else if(brotherNode.getAttribute("tabletype").equals("nest")){
									nowTr++;
								}else if(tmp2.getNodeName().equals("VALUE")||tmp2.getNodeName().equals("EMBED")){
									nowTr++;
							...
							if(brotherNode.hasAttribute("oldtr") && !brotherNode.getAttribute("oldtr").equals("")){
								needTr += Integer.parseInt(brotherNode.getAttribute("oldtr"));
							}else if(brotherNode.getAttribute("tabletype").equals("nest")){
								needTr++;
							}else if(tmp2.getNodeName().equals("VALUE")||tmp2.getNodeName().equals("EMBED")){
								needTr++;
						...
						if((nowTr+needTr) == maxtr){
							if(childNode.hasAttribute("oldtr"))
								childNode.setAttribute("rowspan",childNode.getAttribute("oldtr"));
							else
							...
							if(nowNode.hasAttribute("oldtr")){
								oldTr = Integer.parseInt(nowNode.getAttribute("oldtr"));
							}
							if(childNode.hasAttribute("oldtr")){
								choldTr = Integer.parseInt(childNode.getAttribute("oldtr"));
							}
							int set = maxtr/oldTr * choldTr;
							childNode.setAttribute("rowspan",Integer.toString(set));
						}	
					}
				}
				...
				if(nowNode.hasAttribute("tr")){//trがあるならテーブル<TR>を作成
					...
					if(h.getNodeName() == "TR"){//TRをつくったことがある
						...
						if(ch.getPreviousSibling()!=null){
							...
							if(brNode.hasAttribute("tr")){
								oldtr = Integer.parseInt(brNode.getAttribute("tr"));
							}
						}else{
							oldtr = 0;
						}						
					...
					if(nowNode.getAttribute("tabletype").equals("nest") && !nowNode.getNodeName().equals("VALUE")){
						int i=0;
						for(Node tmp = h;tmp != null;tmp = tmp.getNextSibling()){
							if(i == y){
								par = tmp;
								break;
							}
						...
						Element strtd = null;
						if(!parentNode.getAttribute("tabletype").equals("none")){
							strtd = doc.createElement("TD");
							par.appendChild(strtd);
						}				
						if(nowNode.hasAttribute("valign")){
							strtd.setAttribute("valign", nowNode.getAttribute("valign"));
						}
						if(nowNode.hasAttribute("height")){
							strtd.setAttribute("height", nowNode.getAttribute("height"));
						}
						Element strtable = doc.createElement("TABLE");
						if(nowNode.hasAttribute("tableborder")){
							strtable.setAttribute("border",nowNode.getAttribute("tableborder"));
						}else{
							strtable.setAttribute("border","1");
						}
						if(nowNode.hasAttribute("class")){
							strtable.setAttribute("class", nowNode.getAttribute("class"));
						}
						if(nowNode.hasAttribute("align")){
							strtable.setAttribute("align", nowNode.getAttribute("align"));
						}
						if(nowNode.hasAttribute("cellspacing")){
							strtable.setAttribute("cellspacing",nowNode.getAttribute("cellspacing"));
						}
						if(nowNode.hasAttribute("cellpadding")){
							strtable.setAttribute("cellpadding",nowNode.getAttribute("cellpadding"));
						}
						if(!parentNode.getAttribute("tabletype").equals("none")){
				...
				if(ch.getNodeName() == "EMBED"){
					int colspan = Integer.parseInt(nowNode.getAttribute("colspan"));
					int rowspan = Integer.parseInt(nowNode.getAttribute("rowspan"));
					inputValue(ch,rowspan,colspan,x,y,h);
					valuenum++;
				}
				//値の書き込み
				if(ch.getNodeName() == "VALUE" || nowNode.getAttribute("tabletype").equals("nest")){					
					if(parentNode.getAttribute("type").equals("connect") &&
							parentNode.getAttribute("dimension").equals("1")){/*C1 VALUE*/
						int rowspan = 1,colspan = 1;
						if(nowNode.hasAttribute("colspan")){//maxtdがあるなら
							colspan = Integer.parseInt(nowNode.getAttribute("colspan"));
						}
						if(parentNode.hasAttribute("plustd")){
							if(ch.getParentNode().getLastChild() == ch && nowNode.getAttribute("child").equals(nowNode.getAttribute("val"))){
								colspan +=  Integer.parseInt(parentNode.getAttribute("plustd"));
							}
						}
						if (nowNode.hasAttribute("rowspan")){
							rowspan = Integer.parseInt(nowNode.getAttribute("rowspan"));
						}
						if(nowNode.getAttribute("tabletype").equals("nest") && !ch.getNodeName().equals("VALUE")){
							Element tmp = (Element)par.getParentNode();
							if(rowspan > 1){
								tmp.setAttribute("rowspan", Integer.toString(rowspan));
							}
							if(colspan > 1){
								tmp.setAttribute("colspan", Integer.toString(colspan));
							}
						}else{
							inputValue(ch,rowspan,colspan,x,y,h);
						}
						...
						if(!nowNode.getAttribute("tabletype").equals("nest")){
							valuenum+=colspan;
							nowNode.setAttribute("colspan",Integer.toString(colspan));
							nowNode.setAttribute("rowspan",Integer.toString(rowspan));
						}
					}
					if(parentNode.getAttribute("type").equals("connect") &&
							parentNode.getAttribute("dimension").equals("2")){/*C2 VALUE*/
						int rowspan = 0,colspan = 0;
						if(nowNode.hasAttribute("rowspan")){//oldtrがあるなら
							rowspan = Integer.parseInt(nowNode.getAttribute("rowspan"));
						}
						if(nowNode.hasAttribute("colspan")){//oldtrがあるなら
							colspan = Integer.parseInt(nowNode.getAttribute("colspan"));
						}
						if(nowNode.getAttribute("tabletype").equals("nest") && !ch.getNodeName().equals("VALUE")){
							Element tmp = (Element)par.getParentNode();
				...
				if(nowNode.getAttribute("type").equals("connect") &&
						nowNode.getAttribute("dimension").equals("1")){//nodeがC1のとき
					//rowspanのかきこみ
					if(nowNode.getAttribute("child").equals(nowNode.getAttribute("val"))){
						Element e2 = (Element)(ch.getFirstChild());
						nowNode.setAttribute("rowspan",e2.getAttribute("rowspan"));
					}
				...
				if(nowNode.getAttribute("type").equals("connect") &&
					...
					if(nowNode.getAttribute("child").equals(nowNode.getAttribute("val"))){
						Element e2 = (Element)(ch.getFirstChild());
						nowNode.setAttribute("colspan",e2.getAttribute("colspan"));
					}
				}
	...
	public void inputValue(Node ch,int rowspan,int colspan,int x,int y,Node n){
		String s = null;
		String imgsrc = null;
		Element nowNode = (Element)ch;		
		//System.out.println("NODE: "+nowNode.getFirstChild().getNodeValue());
		if(nowNode.getAttribute("type").equals("img")){//imageのとき
			s = null;
		...
		int i=-1;
		for(Node n1 = n;
		n1 != null && i < y;n1 = n1.getNextSibling()){
		...
		if(nowNode.hasAttribute("href")){//hrefがあるなら
