...
public class VRFunction extends Function {
	...
	private void Func_checkbox() {
		...
		if (!this.getAtt("checked").equals("")) {
			VREnv.setChecked(this.getAtt("checked"));
		}
...
//		}
	...
	private void Func_FormCommon(String s) {
		...
		if (!this.getAtt("name").equals("")) {
			VREnv.setFormPartsName(this.getAtt("name"));
			VREnv.exFormName();
		} else {
			VREnv.setFormPartsName(null);
		}

		if (!this.getAtt("id").equals("")) {
			VREnv.nameId = this.getAtt("id");
		}

		if (!this.getAtt("cond_name").equals("")) {
			VREnv.condName = this.getAtt("cond_name");
		}
		if (!this.getAtt("cond").equals("")) {
			VREnv.cond = this.getAtt("cond");
		}
	...
	private void Func_select() {
		if (!this.getAtt("selected").equals("")) {
			VREnv.setSelected(this.getAtt("selected"));
		}
	...
	protected void Func_imagefile() {
		...
		if (decos.containsKey("lightbox")) {
			...
			if (decos.getStr("lightbox").compareTo("root") == 0
				...
				htmlEnv.code.append("<img class=\"" + VREnv.getClassID(this)
						+ " ");
			...
			if (Ehtml.flag || Incremental.flag) {
				Incremental.outXMLData(htmlEnv.xmlDepth, "<Img class=\'"
						+ VREnv.getClassID(this) + "\' src='" + path + "/"
	...
	protected void Func_submit() {
		...
		// submit only ----- no "@{form}"
		if (!VREnv.getFormItemFlg() && !decos.containsKey("form")) {
			form = createForm();
			openFormInThis = true;
		...
		VREnv.setFormItemFlg(true, "submit");
